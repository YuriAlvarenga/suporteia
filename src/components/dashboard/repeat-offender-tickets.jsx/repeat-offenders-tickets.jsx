...
export default function RepeatOffenderTickets({ monitoredStores = [], onOpenModal }) {
    const { tickets = [] } = useSelector((state) => state.tickets)
    const { companies = [] } = useSelector((state) => state.companies)

    // Estados internos para controle do envolvimento (Paper/Header)
                ...
                <Collapse in={openFilters}>
                    <Box sx={{ mb: 3, p: 2, bgcolor: '#f8f9fa', borderRadius: 2, border: '1px solid #e0e0e0' }}>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
                            ...
                            <ToggleButtonGroup
                                value={days} exclusive onChange={(e, val) => val && setDays(val)}
                                size="small"
                                sx={{
                                    height: 25,
